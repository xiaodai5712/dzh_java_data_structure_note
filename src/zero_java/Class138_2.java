...
public class Class138_2
{
    ...
    {
        System.out.println("线程开始，线程的名字是：" + Thread.currentThread().getName());
        List<Thread> threads = new ArrayList<>();
        for(int i = 1; i<= 1; i ++)
        {
//            Thread thread = new Thread(new PrintStoryRunnable(TEXT,200 * i));
            Thread thread = new Thread(new NotHandleInterrupt(TimeUnit.SECONDS.toMillis(8)),"我的线程");
            thread.start();
            threads.add(thread);
        }
        ...
        System.out.println("interrupt 线程结束，sleep 5秒");
        Thread.sleep(TimeUnit.SECONDS.toMillis(5));
        System.out.println("线程结束，线程的名字是：" + Thread.currentThread().getName());
    }
        ...
        public void run()
        {
            ...
            while(true)
            {
                ...
                if(isInterrupted != Thread.currentThread().isInterrupted())
                {
                    ...
                    System.out.println("发现线程：" + Thread.currentThread().getName() + "的isInterrupted 变化为" + isInterrupted);
                    System.out.println("但是什么都不做");
                }
